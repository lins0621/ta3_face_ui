...
<cn>
#### 隐藏/显示列
ta-big-table使用方法hideColumn、showColumn来控制隐藏显示
</cn>

<us>
#### hide/show columns
Use the table methods hidecolumn and showcolumn to control the hidden display
</us>

```html
<template>
    <div>
        <ta-button @click="hideSex">隐藏sex列</ta-button>
        <ta-button @click="showSex('sex')">显示sex列</ta-button>
        <ta-button @click="showSex('address')">显示Address列</ta-button>
        <ta-button @click="resetColunm">重置</ta-button>
        <ta-big-table
                ref="xTable"
                border
                stripe
            ...
            <ta-big-table-column field="address" title="Address" :visible="false" show-overflow></ta-big-table-column>
        </ta-big-table>
    </div>
</template>
<script>
    export default {
        ...
        methods: {
            ...
            showSex(columnName){
                this.$refs.xTable.showColumn(this.$refs.xTable.getColumnByField(columnName))
            },
            resetColunm(){
